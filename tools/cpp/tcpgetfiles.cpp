...
void _help() {
  ...
  printf(
    "Sample:/project/tools/bin/procctl 20 /project/tools/bin/tcpgetfiles "
  ...
  while (true) {
    ...
    if (strrecvbuffer.find("<filename>") != string::npos) {
      ...
      if (recvfile(clientfilename, mtime, filesize) == true) {
        logfile << "ok.\n";
        sformat(
          strsendbuffer, "<filename>%s</filename><result>ok</result>", serverfilename.c_str()
        );
      } else {
        logfile << "failed.\n";
        sformat(
          strsendbuffer, "<filename>%s</filename><result>failed</result>", serverfilename.c_str()
        );
      }
